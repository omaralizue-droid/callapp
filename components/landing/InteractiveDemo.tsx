  ...
  return (
    <div className="rounded-xl overflow-hidden border border-slate-200 bg-white shadow-md">
      ...
      <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-4">
        <div>
          <span className="text-xs text-indigo-600 font-bold tracking-wider uppercase">Live AI Intelligence</span>
          <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
            Call_Ref_9821_Refund.wav
            <span className="bg-emerald-50 text-emerald-700 text-xs px-2.5 py-0.5 rounded-full font-bold border border-emerald-100">
              Analyzed
            </span>
          </h3>
        </div>
        <div className="flex gap-2">
          <div className="bg-white rounded-lg px-4 py-2 border border-slate-200 text-center shadow-sm">
            <span className="block text-[9px] text-slate-400 font-bold uppercase tracking-wider">QA Score</span>
            <span className="text-lg font-extrabold text-indigo-600">96%</span>
          </div>
          <div className="bg-white rounded-lg px-4 py-2 border border-slate-200 text-center shadow-sm">
            <span className="block text-[9px] text-slate-400 font-bold uppercase tracking-wider">Sentiment</span>
            <span className="text-lg font-extrabold text-emerald-600">Positive</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100 min-h-[420px]">
        ...
        <div className="p-5 flex flex-col justify-between overflow-y-auto col-span-1 bg-slate-50/50">
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-3 text-xs leading-relaxed text-indigo-800">
                <span className="font-bold text-indigo-600 block mb-1">Strength: Active Empathy</span>
                Agent Alex resolved duplicate charges immediately. Displayed strong reassurance and apology statements.
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs leading-relaxed text-slate-700">
                <span className="font-bold text-slate-800 block mb-2">Compliance Checklist</span>
          ...
          <div className="pt-4 border-t border-slate-200 flex items-center justify-between text-xs text-slate-400 font-semibold">
        ...
        <div className="p-5 col-span-2 overflow-y-auto space-y-4 flex flex-col h-full justify-between bg-white">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className={`font-bold ${seg.speaker === 'Agent' ? 'text-indigo-600' : 'text-slate-700'}`}>
                      {seg.speaker}
                    </span>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-slate-400 font-medium">{seg.time}</span>
                      {seg.compliance && (
                        <span className="bg-emerald-50 text-emerald-700 text-[9px] px-1.5 py-0.5 rounded border border-emerald-100 font-bold uppercase tracking-wide">
                          Compliant
                        </span>
                      )}
                      {seg.sentiment === 'positive' && <Smile className="w-3.5 h-3.5 text-emerald-600" />}
                      {seg.sentiment === 'negative' && <Frown className="w-3.5 h-3.5 text-rose-500" />}
                    </div>
                  </div>
                  <p className="text-slate-600 leading-relaxed">{seg.text}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 flex items-center gap-4 mt-2">
            <button
              onClick={togglePlayback}
              className="bg-indigo-600 hover:bg-indigo-700 text-white p-2.5 rounded-full transition-transform hover:scale-105"
            >
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-slate-400">
              ...
              <div className="w-12 bg-slate-200 rounded-full h-1">
                <div className="bg-indigo-600 h-1 rounded-full w-2/3" />
              </div>
            </div>
          </div>
